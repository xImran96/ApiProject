                          ...
                          <div class="col-lg-7">
                              <select id="cat" required="">
                                  <option value="">{{ __('Select Category') }}</option>
                                    @foreach($cats as $cat)
                                      <option data-href="{{ route('admin-subcat-load',$cat->id) }}" value="{{ $cat->id }}" {{ $cat->id == $data->subcategory->category->id ? "selected":"" }}>@if(Session::get('language')==2) {{ $cat->name_ar }} @else {{$cat->name_en}} @endif</option>
                                    @endforeach
                                </select>
                          </div>
                        </div>
                          ...
                          <div class="col-lg-7">
                              <select id="subcat"  name="subcategory_id" required="">
                                ...
                                @foreach($data->subcategory->category->subs as $sub)
                                  <option value="{{$sub->id}}" {{$sub->id == $data->subcategory->id ? "selected":""}}>@if(Session::get('language')==2) {{ $sub->name_ar }} @else {{$sub->name_en}} @endif</option>
                                @endforeach
                              </select>
                          </div>
                        </div>
