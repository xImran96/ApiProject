...
@section('content')  
						...
						<div class="content-area">
							...
							<form id="geniusform" action="{{route('vendor-product-update',$data->id)}}" method="POST" enctype="multipart/form-data">
								{{csrf_field()}}
                        ...
                        @include('includes.admin.form-both')
												...
												<div class="row">
													<div class="col-lg-12">
														<div class="left-area">
																...
																<p class="sub-heading">{{  __('custom.product_name_en') }}</p>
														</div>
													</div>
													<div class="col-lg-12">
														<input type="text" class="input-field" placeholder="{{ __('custom.product_name_en') }}" name="name_en" required="" value="{{ $data->name_en }}">
													</div>
												</div>
												...
												<div class="row">
													<div class="col-lg-12">
														<div class="left-area">
																<h4 class="heading">{{ __('Product Name') }}* </h4>
																<p class="sub-heading">{{  __('custom.product_name_ar') }}</p>
														</div>
													</div>
													<div class="col-lg-12">
														<input type="text" class="input-field" placeholder="{{ __('custom.product_name_ar') }}" name="name_ar" required="" value="{{ $data->name_ar }}">
													</div>
												</div>
                                              ...
                                              @foreach($cats as $cat)
                                                  <option data-href="{{ route('admin-subcat-load',$cat->id) }}" value="{{$cat->id}}" {{$cat->id == $data->category_id ? "selected":""}} >@if(Session::get('language')==2)   {{ $cat->name_ar }} @else  {{ $cat->name_en }} @endif</option>
                                              @endforeach
						                                     </select>
													</div>
												</div>
...
// Gallery Section Update
    ...
    $(document).on("click", ".set-gallery" , function(){
        ...
        $('.selected-image .row').html('');
            $.ajax({
                    type: "GET",
                    url:"{{ route('vendor-gallery-shows') }}",
                    data:{id:pid},
  ...
  $(document).on('click', '.remove-img' ,function() {
    ...
    $(this).parent().parent().remove();
	    $.ajax({
	        type: "GET",
	        url:"{{ route('vendor-gallery-deletes') }}",
	        data:{id:id}
	    });
  });
  ...
  $(document).on('submit', '#form-gallery' ,function() {
		  $.ajax({
		   url:"{{ route('vendor-gallery-stores') }}",
		   method:"POST",
...
$('.cropme').simpleCropper();
 ...
 setTimeout(
    ...
    function() {
      ...
      $.ajax({
        url: "{{route('vendor-product-upload-updates',$data->id)}}",
        type: "POST",
        data: {"image":img},
        success: function (data) {
          if (data.status) {
