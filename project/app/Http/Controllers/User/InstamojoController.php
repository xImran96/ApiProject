...
$result = $exchangeRates->exchangeRate('USD', 'INR', Carbon::now());
    ...
    $user = Auth::user();
     ...
     // $result = intval(($result * $subs->price));
     // dd($result);
     $settings = Generalsetting::findOrFail(1);
     $paypal_email = $settings->paypal_business;
     $return_url = action('User\PaypalController@payreturnUpgrade');
     $cancel_url = action('User\PaypalController@paycancleUpgrade');
     $notify_url = action('User\InstamojoController@notifyUpgrade');
     $item_name = $subs->title." Plan";
     $item_number = Str::random(10);
     $item_amount = intval(($result * $subs->price));
     Session::put('user_data',$input);
...
try {
    ...
    $redirect_url = $response['longurl'];
     ...
     Session::put('upgrade-subscription',$sub);
...
public function notifyUpgrade(Request $request){

    $data = $request->all();



        $sub = Session::get('upgrade-subscription');
        ...
        $paypal_data = Session::get('paypal_data');
        $paypal_items = Session::get('paypal_items');
        $success_urlUpgrade = action('User\PaypalController@payreturnUpgrade');
        $cancel_urlUpgrade = action('User\PaypalController@paycancleUpgrade');


        if($sub['pay_id'] == $data['payment_request_id']){


                    $order = UserSubscription::find($package->id);
                    $order->user_id = $sub['user_id'];
                    $order->subscription_id = $sub['subscription_id'];
                    $order->title = $sub['title'];
                    $order->currency = $sub['currency'];
...
Session::forget('upgrade-subscription');

            return redirect($success_urlUpgrade);
        }
        else {
            return redirect($cancel_urlUpgrade);
        }
