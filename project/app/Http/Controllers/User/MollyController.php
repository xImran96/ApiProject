...
//Upgrade Plan
 ...
 public function update(Request $request){
    ...
    $this->validate($request, [
     ...
     $notify_url = action('User\MollyController@notifyUpgrade');
     $order['item_name'] = $subs->title." Plan";
     $order['item_number'] = Str::random(10);
     $order['item_amount'] = $subs->price;
    ...
    $settings = Generalsetting::findOrFail(1);
        ...
        $payment = Mollie::api()->payments()->create([
            ...
            'description' => $order['item_name'] ,
            'redirectUrl' => route('user.molly.notifyUpgrade'),
            ]);
...
public function notifyUpgrade(Request $request){
        ...
        $success_url = action('User\PaypalController@payreturnUpgrade');
        $cancel_url = action('User\PaypalController@paycancleUpgrade');
        ...
        {
            $headers = "From: ".$settings->from_name."<".$settings->from_email.">";
            mail($user->email,'Your Vendor Plan Upgraded','Your Vendor Plan Upgraded Successfully. Please Login to your account and build your own shop.',$headers);
        }
