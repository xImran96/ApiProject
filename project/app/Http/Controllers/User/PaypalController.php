...
// Set TCP timeout to 30 seconds
 ...
 public function update(Request $request){
     ...
     $notify_url = action('User\PaypalController@notifyUpgrade');
     $item_name = $subs->title." Plan";
     $item_number = Str::random(10);
     $item_amount = $subs->price;
    ...
    $userSub = auth()->user()->subscribes()->where('status',1)->orderBy('id','desc')->first();

    $sub = UserSubscription::find($userSub->id);
    $sub->user_id = $user->id;
    $sub->subscription_id = $subs->id;
    $sub->title = $subs->title;
    $sub->currency = $subs->currency;
    ...
    dd($querystring);
     ...
     public function paycancleUpgrade(){
         return redirect()->back()->with('unsuccess','Payment Cancelled.');
     }

     public function payreturnUpgrade(){
...
// Set TCP timeout to 30 seconds
    ...
    if (strcmp($res, "VERIFIED") == 0 || strcasecmp($res, "VERIFIED") == 0) {
        ...
        {
            $headers = "From: ".$settings->from_name."<".$settings->from_email.">";
            mail($user->email,'Your Vendor Plan Upgraded Successfully','Please Login to your account and build your own shop.',$headers);
        }
