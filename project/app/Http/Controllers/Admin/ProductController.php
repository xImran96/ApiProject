...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
       ...
       // dd($request);
        ...
        if($request->type == "Physical")
        {
            ...
            //--- Validation Section
            $rules = ['sku'=> 'min:6|unique:products'];
        ...
        if($prod->type != 'Physical'){
            $prod->slug = Str::slug($data->name_en,'-').'-'.strtolower(Str::random(3).$data->id.Str::random(3));
        }
        else {
   ...
   // add cateogry ids rather than name 
        $validator = Validator::make($request->all(), $rules);
        ...
        while (($line = fgetcsv($file)) !== FALSE) {
            ...
            if (!Product::where('sku',$line[0])->exists()){
               ...
               // $mcat = Category::where(DB::raw('lower(name)'), strtolower($line[1]));
                $mcat = Category::whereId($line[1]);

                if($mcat->exists()){
                    $input['category_id'] = $line[1];

                    if($line[2] != ""){
                        $scat = Subcategory::whereId($line[2]);

                        if($scat->exists()) {
                            $input['subcategory_id'] = $line[2];
                        }
                    }
                    if($line[3] != ""){
                        $chcat = Childcategory::whereId($line[3]);

                        if($chcat->exists()) {
                            $input['childcategory_id'] = $line[3];
                        }
                    }
                ...
                $input['color'] = $line[13];
                $input['price'] = $line[8];
                $input['previous_price'] = $line[9] != "" ? $line[9] : null;
                $input['stock'] = $line[9];
                $input['size'] = $line[11];
                ...
                $input['slug'] = Str::slug($input['name_en'],'-').'-'.strtolower($input['sku']);
                ...
                $thumb_url = '';
                    ...
                    if (strpos($contentType, 'image/') !== false) {
                        $fimg = Image::make($line[6])->resize(800, 800);
